...
// ===== Demo Admin Auth (NOT SECURE) =====
const ADMIN_USER = "adminahmed";
const ADMIN_PASS = "adminroot";
const SESSION_KEY = "ds_admin_session_v2";

// ===== Storage keys =====
const STORE_KEY = "ds_club_store_v2";     // current working store
...
function hasSession(){
  try{
    ...
    return s.ok === true && (Date.now() - s.at) < 12*60*60*1000; // 12 hours
  }catch{
    return false;
  }
}
...
function loadMemberToForm(m){
  ...
  m_msg.textContent = "تم تحميل العضو للتعديل.";
...
function renderMemberList(){
  ...
  m_meta.textContent = `الأعضاء: ${list.length} — آخر تحديث للـ Store: ${store.updatedAt ?? "—"}`;
}
...
function renderGroupList(){
  ...
  g_meta.textContent = `المجموعات: ${list.length}`;
}
...
g_save.addEventListener("click", ()=>{
  ...
  if(!sport || !name){
    g_msg.textContent = "لازم تكتب الرياضة + اسم المجموعة.";
    return;
  }
  if((minAge != null && Number.isNaN(minAge)) || (maxAge != null && Number.isNaN(maxAge))){
    g_msg.textContent = "العمر لازم يكون رقم.";
    return;
  }
  if(minAge != null && maxAge != null && minAge > maxAge){
    g_msg.textContent = "Min Age لازم يكون أقل/يساوي Max Age.";
    return;
  }
  ...
  if(currentGroupId){
    ...
    if(idx === -1){
      g_msg.textContent = "المجموعة الحالية غير موجودة.";
      return;
    }
    store.groups[idx] = {
      ...store.groups[idx],
    ...
    saveStore(store);
    g_msg.textContent = "تم تحديث المجموعة ✅";
  }else{
    const id = randId("G");
    store.groups.push({
    ...
    g_msg.textContent = "تم إضافة المجموعة ✅";
  }
...
g_delete.addEventListener("click", ()=>{
  if(!currentGroupId){
    g_msg.textContent = "مفيش مجموعة مختارة للحذف.";
    return;
  }
  const membersUsing = store.members.filter(m => (m.groupId || "") === currentGroupId).length;
  const ok = confirm(membersUsing
    ? `المجموعة عليها ${membersUsing} لاعب. لو حذفتها هيتشال الربط من اللاعبين. متأكد؟`
  ...
  g_msg.textContent = "تم حذف المجموعة ✅";
  resetGroupForm();
  refreshGroupDropdowns();
  renderGroupList();
  renderMemberList();

  // also if current member linked, update form dropdown
  if(currentMemberId){
...
m_save.addEventListener("click", ()=>{
  ...
  if(!name){
    m_msg.textContent = "لازم تكتب اسم العضو.";
    return;
  }
  if(age != null && (Number.isNaN(age) || age < 0)){
    m_msg.textContent = "السن لازم يكون رقم صحيح.";
    return;
  }
  if(Number.isNaN(debt)){
    m_msg.textContent = "المديونية لازم تكون رقم.";
    return;
  }
  ...
  if(currentMemberId){
    ...
    if(idx === -1){
      m_msg.textContent = "العضو الحالي غير موجود.";
      return;
    }
    store.members[idx] = {
      ...store.members[idx],
    ...
    store.updatedAt = nowIso();
    saveStore(store);
    m_msg.textContent = "تم تحديث العضو ✅";
  }else{
    const id = randId("M");
    store.members.push({
    ...
    m_msg.textContent = "تم إضافة العضو ✅";
  }
...
m_delete.addEventListener("click", ()=>{
  if(!currentMemberId){
    m_msg.textContent = "مفيش عضو مختار للحذف.";
    return;
  }
  const ok = confirm("متأكد إنك عايز تحذف العضو؟");
  if(!ok) return;
...
p_add.addEventListener("click", ()=>{
  if(!currentMemberId){
    m_msg.textContent = "اختار عضو الأول (Load) أو اعمل Save لعضو جديد.";
    return;
  }
  ...
  if(!amount || !date){
    m_msg.textContent = "لازم Amount + Date.";
    return;
  }

  const idx = store.members.findIndex(x => x.id === currentMemberId);
  if(idx === -1){
    m_msg.textContent = "العضو غير موجود.";
    return;
  }
  ...
  p_amount.value = "";
  p_date.value = "";
  m_msg.textContent = "تم إضافة الدفعة ✅";
...
ex_download.addEventListener("click", ()=>{
  ...
  ex_msg.textContent = "تم تنزيل club-data.json ✅ (ارفعه للريبو علشان يظهر في Public)";
});

ex_copy.addEventListener("click", async ()=>{
  const text = exportJson();
  await copyText(text);
  ex_msg.textContent = "تم نسخ JSON ✅";
});
...
ex_loadFromRepo.addEventListener("click", async ()=>{
  ...
  try{
    ...
    ex_msg.textContent = "تم تحميل club-data.json من الريبو إلى التخزين المحلي ✅";
  }catch{
    ex_msg.textContent = "فشل تحميل club-data.json — تأكد إنه موجود في الريبو.";
  }
});
...
ex_importFile.addEventListener("change", async (e)=>{
  ...
  try{
    ...
    ex_msg.textContent = "تم Import ✅";
  }catch{
    ex_msg.textContent = "Import فشل — الملف مش JSON صحيح.";
  }finally{
    ex_importFile.value = "";
  }
});
...
function renderBackups(){
  ...
  list.forEach(bk=>{
    ...
    tr.querySelector(".btnRestore").addEventListener("click", ()=>{
      const ok = confirm("Restore هذا الـ Backup؟ (هيستبدل البيانات الحالية)");
      if(!ok) return;
...
bk_create.addEventListener("click", ()=>{
  ...
  saveBackups(backups);
  renderBackups();
  bk_msg.textContent = "تم إنشاء Backup ✅";
});

bk_clear.addEventListener("click", ()=>{
  const ok = confirm("مسح كل الـ Backups؟");
  if(!ok) return;
  backups = [];
  saveBackups(backups);
  renderBackups();
  bk_msg.textContent = "تم مسح كل الـ Backups ✅";
});
